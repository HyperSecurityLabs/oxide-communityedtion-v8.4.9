...
impl WebCrawler {
    ...
    pub async fn crawl(&mut self, start_url: &str) -> Result<CrawlResult> {
        ...
        while let Some((url, depth)) = self.queue.pop_front() {
            ...
            tokio::spawn(async move {
                ...
                while !s.load(std::sync::atomic::Ordering::Relaxed) {
                    ...
                    idx += 1;
                    print!("\r  {} {} fetching  depth:{}  {}  [{:02}:{:02}]",
                        tc("[*]", OSAKA_JADE),
                        tc(frame, OSAKA_JADE_B),
                        tc(&depth_val.to_string(), LAVENDER_BLUE),
            ...
            match self.client.send(request).await {
                Ok(response) => {
                    ...
                    print!("\r\x1B[2K");
                    println!("  {} {} {}  depth:{} forms:{} links:{}  {}  [{:02}:{:02}]",
                        tc("[*]", OSAKA_JADE),
                        tc(&status.to_string(), OSAKA_JADE_B),
                        tc(&size_str, LAVENDER_BLUE),
                        tc(&depth.to_string(), LAVENDER_BLUE),
                        tc(&new_forms.to_string(), LAVENDER_BLUE),
                        tc(&new_links.to_string(), LAVENDER_BLUE),
        ...
        println!("  {} Crawl complete: {} pages, {} URLs, {} forms, {} links",
            tc("[+]", OSAKA_JADE),
            tc(&page_count.to_string(), OSAKA_JADE_B),
            tc(&total.to_string(), OSAKA_JADE_B),
            tc(&self.forms.len().to_string(), OSAKA_JADE_B),
            tc(&self.links.len().to_string(), OSAKA_JADE_B));
        Ok(result)
    }
    ...
    async fn extract_forms(&mut self, url: &str, body: &str) -> Result<()> {
        ...
        for form_m in form_re.find_iter(body) {
            ...
            let action = self.resolve_url(url, &action)
...
impl Clone for WebCrawler {
    fn clone(&self) -> Self {
        let client = HttpClient::new(crate::http::client::HttpClientConfig {
            ...
            max_redirects: 10,
        }).expect("HttpClient::new in WebCrawler::clone");
        Self {
            client,
