...
impl HybridScanner {
    pub fn new(args: CliArgs) -> Result<Self> {
        ...
        if args.rate_limit > 0 {
            let delay = Duration::from_millis(1000 / args.rate_limit);
            crawler.set_rate_limit(delay);
        }
    ...
    async fn scan_all(&mut self) -> Result<Vec<Finding>> {
        ...
        println!("  {} {} {}",
            tc("◆", EV_GRN_B),
            tc("Engines initialised — starting scan with", EV_FG),
            tc(&modules.join(", "), EV_YLW_B).bold());
        Output::print_line();
...
// ── Evergreens-themed output helpers ─────────────────────────────────────

use crate::cli::display::{
    EV_AQU, EV_BLU, EV_BLU_B, EV_FG, EV_FG0, EV_GRN_B, EV_GRY,
    EV_ORG_B, EV_RED, EV_RED_B, EV_YLW, EV_YLW_B,
};
...
fn fmt_status(status: u16) -> String {
    match status {
        200..=299 => tc(&status.to_string(), EV_GRN_B),
        300..=399 => tc(&status.to_string(), EV_YLW),
        400..=499 => tc(&status.to_string(), EV_RED),
        500..=599 => tc(&status.to_string(), EV_RED_B),
        _ => tc(&status.to_string(), EV_GRY),
    }
}

fn fmt_sev_label(severity: &Severity) -> String {
    match severity {
        Severity::Critical => tc("▌CRITICAL▐", EV_RED_B),
        Severity::High     => tc("▌ HIGH   ▐", EV_RED),
        Severity::Medium   => tc("▌ MEDIUM ▐", EV_YLW),
        Severity::Low      => tc("▌ LOW    ▐", EV_AQU),
        Severity::Info     => tc("▌ INFO   ▐", EV_BLU),
    }
}
