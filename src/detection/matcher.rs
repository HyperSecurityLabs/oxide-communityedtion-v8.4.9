...
impl Matcher {
    pub fn new() -> Self {
        ...
        patterns.insert(
            "sql_error".to_string(),
            Regex::new(r"(SQL syntax|mysql_fetch|mysqli_fetch|ORA-|PostgreSQL|SQLite|SQLServer|Unclosed quotation|Incorrect syntax|SQLSTATE|pg_query|ODBC Driver|Microsoft OLE DB|java\.sql\.|syntax error|Warning.*mysql|You have an error)")
                .expect("Static SQL error regex should be valid")
        );
        
        patterns.insert(
            "xss_vulnerable".to_string(),
            Regex::new(r"(javascript:|onload\s*=|onerror\s*=|onfocus\s*=|onclick\s*=|onmouseover\s*=)")
