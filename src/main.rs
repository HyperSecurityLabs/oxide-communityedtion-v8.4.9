...
use crate::cli::display::{
    GB_AQU, GB_AQU_B, GB_BLU_B, GB_FG, GB_FG0, GB_GRN_B, GB_GRY, GB_RED_B, GB_YLW, GB_YLW_B,
};
...
fn print_banner() {
    ...
    println!("  {} {}",
        tc("◈", GB_YLW_B),
        tc("Hypersecurity Offensive Labs  |  OXIDE Community Edition v8.4.9", GB_FG0));
    println!("  {} {}",
        tc(">>", GB_AQU),
...
async fn print_scan_info(args: &CliArgs) {
    ...
    use crate::cli::display::{
        GB_AQU, GB_BLU_B, GB_FG, GB_FG0, GB_GRN_B, GB_GRY, GB_RED_B, GB_YLW_B,
    };

    Output::print_header("Target Information");
    if args.multiattack_enabled() {
        println!("  {} {}  {} {}",
            tc("▸", GB_YLW_B), tc("Multi-Attack", GB_GRY).bold(),
            tc("→", GB_GRY), tc(&format!("{} targets", args.target_count()), GB_RED_B));
        ...
        for (i, url) in args.url.iter().enumerate() {
            ...
            let _host = url::Url::parse(&clean)
                ...
                .unwrap_or_default();
            println!("  {} {}  {} {}  {} {}",
                tc("▸", GB_YLW_B), tc(&format!("Target {}", i + 1), GB_GRY).bold(),
                tc("→", GB_GRY), tc(&clean, GB_FG0),
                tc("≈", GB_YLW_B), tc(&format!("{} thr", per_target), GB_GRN_B));
        }
        println!("  {} {}  {} {}  {} {}",
            tc("▸", GB_YLW_B), tc("Threads", GB_GRY).bold(),
            tc("→", GB_GRY), tc(&format!("{} total", args.threads), GB_GRN_B),
            tc("⏱", GB_YLW_B), tc(&format!("{}s duration", args.duration), GB_FG));
    } else {
        ...
        println!("  {} {}  {} {}",
            ...
            tc("→", GB_GRY), args.target_url().truecolor(GB_FG0.0, GB_FG0.1, GB_FG0.2).bold());
        println!("  {} {}  {} {}",
    ...
    if args.zeroday  { println!("  {} {}", tc("▸", GB_YLW_B), tc("Zero-day detection", GB_YLW_B)); }
    if args.train    { println!("  {} {}", tc("▸", GB_YLW_B), tc("Training mode", GB_RED_B)); }
...
async fn main() {
    ...
    if is_multi {
        println!("  {} {}  {} {}",
            tc("⚔", GB_YLW_B),
            tc("Multi-Attack engaged", GB_RED_B).bold(),
            tc("→", GB_GRY),
            tc(&format!("{} concurrent targets", total_targets), GB_FG0));
    }
    println!("  {} {}",
        tc("◈", GB_YLW_B),
    ...
    let (findings, hybrid_scanner): (Vec<_>, _) = if is_multi {
        ...
        for (i, target_url) in args.url.iter().enumerate() {
            ...
            match HybridScanner::new(target_args) {
                Ok(scanner) => {
                    println!("  {} {}  {}",
                        tc("▶", GB_GRN_B),
                        tc(&format!("[Target {}]", i + 1), GB_YLW_B),
                        tc(target_url, GB_FG0));
        ...
        for (idx, mut scanner, target_url) in scanners {
            match scanner.run_hybrid_scan().await {
                Ok(f) => {
                    println!("  {} {}  {}",
                        tc("✓", GB_GRN_B),
                        tc(&format!("[Target {}] done", idx), GB_AQU),
                        tc(&format!("{} findings", f.len()), GB_GRN_B));
        ...
        println!("  {} {}", tc("⚔", GB_YLW_B), tc(&summary, GB_FG0));
        (all_findings, None)
    ...
    println!("  {} {}    {} {}",
        tc("⏱", GB_YLW_B), tc(&format!("Duration: {}", final_duration), GB_FG),
